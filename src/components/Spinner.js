...
export default function Programmers() {
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (programmer | programmerID === null) ? 'royalblue' : 'gold' // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setProgrammer(dev.name); setProgrammerId(dev.id); /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          programmer && programmerID !=null ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
        }
      </div>
    </div>
  );
